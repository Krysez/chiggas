...
try {
  if (!fs.existsSync(SERVER_PATH)) {
    return out({
      ok: false,
      pass: PASS,
      status: 'backend_pass_87b_apply_failed',
  ...
  if (!server.includes("String(process.env.STEAM_PUBLISHER_WEB_API_KEY || '').trim()")) {
    return out({
      ...
      pass: PASS,
      status: 'backend_pass_87b_apply_failed',
      error: 'Could not patch STEAM_PUBLISHER_WEB_API_KEY line. The server.js structure changed.'
    }, 1);
  }
...
`;
  ...
  out({
    ok: true,
    pass: PASS,
    status: 'backend_pass_87b_key_trim_and_health_diagnostics_applied',
    root: ROOT,
    backupRoot,
    changed: {
  ...
  out({
    ok: false,
    pass: PASS,
    status: 'backend_pass_87b_apply_exception',
