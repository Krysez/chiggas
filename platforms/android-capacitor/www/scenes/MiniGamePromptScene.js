...
const MINI_GAME_STORAGE = {
    memory_match: 'chiggas_memory_match_v1',
    parasite_maze: 'chiggas_parasite_maze_v1'
};

const MINI_GAME_LABELS = {
    memory_match: 'MEMORY MATCH',
    parasite_maze: 'PARASITE MAZE'
};

export default class MiniGamePromptScene extends Phaser.Scene {
    constructor() {
        super('MiniGamePromptScene');
    }

    init(data = {}) {
        this.targetGameData = data.targetGameData || { stageIndex: 0, difficulty: 1, controlMode: 'touch' };
        this.triggerStageIndex = data.triggerStageIndex ?? null;
        this.selectedMiniGame = data.miniGameId || this._pickMiniGame();
    }
    ...
    create() {
        ...
        bg.fillGradientStyle(0x1a001a, 0x080008, 0x000000, 0x000000, 1, 1, 1, 1);
        bg.fillRect(0, 0, width, height);

        const panelW = Math.min(620, safe.width - 24);
        const panelH = Math.min(360, safe.height - 32);
        const panelX = safe.centerX;
        const panelY = safe.centerY;

        const panel = this.add.graphics();
        panel.fillStyle(0x111111, 0.98);
        panel.fillRoundedRect(panelX - panelW / 2, panelY - panelH / 2, panelW, panelH, 24);
        panel.lineStyle(6, 0xffdd00, 0.88);
        panel.strokeRoundedRect(panelX - panelW / 2, panelY - panelH / 2, panelW, panelH, 24);

        this.add.text(panelX, panelY - panelH / 2 + 62, 'PLAY MINI-GAME?', {
            fontSize: height < 560 ? '34px' : '54px',
            fontFamily: 'Arial Black, Impact, Dhurjati, sans-serif',
            color: '#ffdd00',
            stroke: '#000000',
            strokeThickness: height < 560 ? 8 : 11,
            align: 'center'
        }).setOrigin(0.5);

        const selectedLabel = MINI_GAME_LABELS[this.selectedMiniGame] || 'MINI-GAME';

        this.add.text(panelX, panelY - 34, selectedLabel, {
        ...
        this.add.text(panelX, panelY + 18, 'Clear it to unlock it from the Mini-Games menu. Skip it to continue the run.', {
        ...
        this._createButton(panelX - 120, panelY + panelH / 2 - 64, 'YES', 0x225522, () => this._startSelectedMiniGame(), 190, 52, 24);
        this._createButton(panelX + 120, panelY + panelH / 2 - 64, 'NO', 0xaa1111, () => this._continueRun(), 190, 52, 24);
    ...
    _handleResize() {
        this.scene.restart({
            targetGameData: this.targetGameData,
            triggerStageIndex: this.triggerStageIndex,
            miniGameId: this.selectedMiniGame
        });
    }

    _pickMiniGame() {
        const games = ['memory_match', 'parasite_maze'];
        const locked = games.filter(id => !this._isStoryUnlocked(id));
        const pool = locked.length > 0 ? locked : games;
        return pool[Math.floor(Math.random() * pool.length)];
    }

    _isStoryUnlocked(id) {
        try {
            const raw = window.localStorage.getItem(MINI_GAME_STORAGE[id]);
    ...
    _startSelectedMiniGame() {
        if (this.selectedMiniGame === 'memory_match') {
            this.scene.start('MemoryMatchScene', {
                storyMode: true,
                returnScene: 'StageIntroScene',
                returnData: { targetGameData: this.targetGameData }
            });
            return;
        }

        if (this.selectedMiniGame === 'parasite_maze') {
            this.scene.start('ParasiteMazeScene', {
                storyMode: true,
                returnScene: 'StageIntroScene',
                returnData: { targetGameData: this.targetGameData }
            });
    ...
    _continueRun() {
        this.scene.start('StageIntroScene', { targetGameData: this.targetGameData });
    }

    _createButton(x, y, text, color, onClick, w = 190, h = 52, fz = 24) {
        ...
        btn.on('pointerdown', () => {
            this.tweens.add({ targets: btn, scaleX: 0.92, scaleY: 0.92, duration: 90, yoyo: true, onComplete: onClick });
        });
