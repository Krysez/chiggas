...
function installFirstDeathPreload() {
  // CHIGGAS_STEAM_PASS_65_FIRST_DEATH_PRELOAD_BEGIN
      (() => {
        const PASS = 'steam_desktop_wrapper_pass_65';
        const APP_ID = 4788490;
        const ACHIEVEMENT = 'FIRST_DEATH';
        const STORE_SHOULD_SHOW = 'TEST BUY';
        const TRACE_FILE = 'steam-achievement-first-death-pass-65.json';
        const LOG_FILE = 'steam-achievement-first-death-pass-65.log';
        ...
        function writeTrace(entry) {
          const full = {
            ...
            pass: PASS,
            appId: APP_ID,
            status: entry.status || 'steam_achievement_pass_65_first_death_trace_event',
            achievement: ACHIEVEMENT,
            triggerInstalled: true,
            triggerMode: 'handlePlayerDeath_side_effect_custom_event_pass32_ipc',
        ...
        async function unlockFromDetail(rawDetail, eventName) {
          ...
          attempted = true;
          const metadata = {
            source: detail.source || 'GameScene_handlePlayerDeath_side_effect',
            scene: detail.scene || 'GameScene',
            event: detail.event || 'first_death_player_death',
            reason: detail.reason || 'handlePlayerDeath_set_isDead_true_playDeath_called',
            isDead: detail.isDead ?? null,
            elapsedTime: detail.elapsedTime ?? null,
            stageIndex: detail.stageIndex ?? null,
            storeShouldShow: STORE_SHOULD_SHOW,
            pass: PASS,
          ...
          writeTrace({
            ok: false,
            status: 'steam_achievement_pass_65_first_death_event_received_waiting_for_ipc',
          ...
          if (!ipc || typeof ipc.invoke !== 'function') {
            return writeTrace({
              ok: false,
              status: 'steam_achievement_pass_65_first_death_ipc_unavailable',
          ...
          try {
            ...
            return writeTrace({
              ...
              status: result && result.status ? result.status : 'steam_achievement_pass_65_first_death_ipc_result',
              attemptedUnlock: true,
              ...
              eventName,
              reason: 'pass32_achievement_ipc_result_65_fixed',
              isDead: metadata.isDead,
              elapsedTime: metadata.elapsedTime,
              stageIndex: metadata.stageIndex,
              metadata,
            ...
            return writeTrace({
              ok: false,
              status: 'steam_achievement_pass_65_first_death_ipc_exception',
              attemptedUnlock: true,
              bridgeChannel: BRIDGE_CHANNEL,
              eventName,
        ...
        function register() {
          try {
            if (typeof window === 'undefined' || window.__chiggasSteamPass65FirstDeathInstalled) return;
            window.__chiggasSteamPass65FirstDeathInstalled = true;
            window.addEventListener('chiggas-steam-achievement-unlock-request', (event) => unlockFromDetail(event && event.detail, 'chiggas-steam-achievement-unlock-request'));
            window.addEventListener('chiggas-first-death', (event) => unlockFromDetail(event && event.detail, 'chiggas-first-death'));
            writeTrace({
              ok: true,
              status: 'steam_achievement_pass_65_first_death_event_bridge_registered',
              attemptedUnlock: false,
            ...
            writeTrace({
              ok: false,
              status: 'steam_achievement_pass_65_first_death_register_failed',
              attemptedUnlock: false,
        ...
        if (document.readyState === 'loading') {
          ...
          setTimeout(register, 750);
        } else {
          register();
        }
      })();
      // CHIGGAS_STEAM_PASS_65_FIRST_DEATH_PRELOAD_END
}

module.exports = {
  installFirstDeathPreload
};
