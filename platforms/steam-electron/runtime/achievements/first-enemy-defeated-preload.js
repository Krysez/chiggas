...
function installFirstEnemyDefeatedPreload() {
  // CHIGGAS_STEAM_PASS_60_FIRST_ENEMY_DEFEATED_PRELOAD_BEGIN
      (() => {
        const PASS = 'steam_desktop_wrapper_pass_60';
        const APP_ID = 4788490;
        const ACHIEVEMENT = 'FIRST_ENEMY_DEFEATED';
        const STORE_SHOULD_SHOW = 'TEST BUY';
        const CHANNEL = 'chiggas-steam-achievements-pass-32-unlock';
        const TRACE_FILE = 'steam-achievement-first-enemy-defeated-pass-60.json';
        const LOG_FILE = 'steam-achievement-first-enemy-defeated-pass-60.log';
        ...
        function writeTrace(extra) {
          const base = {
            ...
            pass: PASS,
            appId: APP_ID,
            status: extra && extra.status ? extra.status : 'steam_achievement_pass_60_first_enemy_defeated_trace_update',
            achievement: ACHIEVEMENT,
            triggerInstalled: true,
            triggerMode: 'chigga_enemy_death_side_effect_custom_event_pass32_ipc',
        ...
        window.addEventListener(EVENT_NAME, async (event) => {
          ...
          writeTrace({
            ok: false,
            status: 'steam_achievement_pass_60_first_enemy_defeated_event_received_waiting_for_ipc',
          ...
          if (!ipcRenderer || typeof ipcRenderer.invoke !== 'function') {
            writeTrace({
              ok: false,
              status: 'steam_achievement_pass_60_ipc_unavailable',
              eventName: EVENT_NAME,
          ...
          try {
            const result = await ipcRenderer.invoke(CHANNEL, ACHIEVEMENT, {
              ...
              event: metadata.event || 'first_enemy_defeated_chigga_death',
              reason: metadata.reason || 'enemy_or_wild_chigga_isDead_set_true',
              faction: metadata.faction || null,
              isWildFaction: metadata.isWildFaction ?? null,
              isEnemyFaction: metadata.isEnemyFaction ?? null
            });
            writeTrace({
              ...
              status: result && result.status ? result.status : 'steam_achievement_pass_60_first_enemy_defeated_ipc_result',
              eventName: EVENT_NAME,
              ...
              reason: 'pass32_achievement_ipc_result_60_fixed',
              faction: metadata.faction || null,
              isWildFaction: metadata.isWildFaction ?? null,
              isEnemyFaction: metadata.isEnemyFaction ?? null,
              metadata
            ...
            writeTrace({
              ok: false,
              status: 'steam_achievement_pass_60_first_enemy_defeated_ipc_exception',
              eventName: EVENT_NAME,
              attemptedUnlock: true,
              bridgeChannel: CHANNEL,
        ...
        writeTrace({
          ok: true,
          status: 'steam_achievement_pass_60_first_enemy_defeated_event_bridge_registered',
          reason: 'preload_registered'
        });
      })();
      // CHIGGAS_STEAM_PASS_60_FIRST_ENEMY_DEFEATED_PRELOAD_END
}

module.exports = {
  installFirstEnemyDefeatedPreload
};
