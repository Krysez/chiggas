...
function installFirstMunchCounterPreload() {
  // CHIGGAS_STEAM_PASS_57_FIRST_MUNCH_COUNTER_PRELOAD_BEGIN
      (() => {
        const PASS = 'steam_desktop_wrapper_pass_57';
        ...
        const TRACE_FILE = 'steam-achievement-first-munch-pass-57.json';
        const LOG_FILE = 'steam-achievement-first-munch-pass-57.log';
        ...
        function writeTrace(extra) {
          const base = {
            ...
            pass: PASS,
            appId: APP_ID,
            status: extra && extra.status ? extra.status : 'steam_achievement_pass_57_first_munch_trace_update',
            achievement: ACHIEVEMENT,
            triggerInstalled: true,
            triggerMode: 'army_level_eat_counter_progress_custom_event_pass32_ipc',
        ...
        window.addEventListener(EVENT_NAME, async (event) => {
          ...
          writeTrace({
            ok: false,
            status: 'steam_achievement_pass_57_first_munch_event_received_waiting_for_ipc',
          ...
          if (!ipcRenderer || typeof ipcRenderer.invoke !== 'function') {
            writeTrace({
              ok: false,
              status: 'steam_achievement_pass_57_ipc_unavailable',
              eventName: EVENT_NAME,
          ...
          try {
            const result = await ipcRenderer.invoke(CHANNEL, ACHIEVEMENT, {
              ...
              event: metadata.event || 'first_munch_eat_counter_progress',
              reason: metadata.reason || 'army_level_eat_counter_progress_advanced'
            });
      
            writeTrace({
              ok: !!(result && result.ok !== false),
              status: result && result.status ? result.status : 'steam_achievement_pass_57_first_munch_ipc_result',
              eventName: EVENT_NAME,
              ...
              reason: 'pass32_achievement_ipc_result_57_fixed',
              xp: metadata.xp ?? null,
              required: metadata.required ?? null,
              level: metadata.level ?? null,
              metadata
            ...
            writeTrace({
              ok: false,
              status: 'steam_achievement_pass_57_first_munch_ipc_error',
              eventName: EVENT_NAME,
              attemptedUnlock: true,
              bridgeChannel: CHANNEL,
        ...
        writeTrace({
          ok: true,
          status: 'steam_achievement_pass_57_first_munch_event_bridge_registered',
          reason: 'preload_registered'
        });
      })();
      // CHIGGAS_STEAM_PASS_57_FIRST_MUNCH_COUNTER_PRELOAD_END
}

module.exports = {
  installFirstMunchCounterPreload
};
