...
function installLeaderboardAutoSubmitMain() {
  // ---- CHIGGAS_LEADERBOARD_AUTO_SUBMIT_PASS_96E_MAIN BEGIN ----
    ;(() => {
      if (global.__CHIGGAS_LEADERBOARD_AUTO_SUBMIT_PASS_96E_MAIN__) return;
      global.__CHIGGAS_LEADERBOARD_AUTO_SUBMIT_PASS_96E_MAIN__ = true;
      ...
      const fs = require('fs');
      const path = require('path');
      const traceFile = path.join(STEAM_ROOT, 'pass96e-leaderboard-auto-submit-trace.json');
      ...
      function appendTrace(kind, data) {
        ...
        trace.submittedRunSignatures = trace.submittedRunSignatures && typeof trace.submittedRunSignatures === 'object' ? trace.submittedRunSignatures : {};
        if (kind === 'request') {
          trace.lastRequest = data;
          trace.requests.push(data);
          if (trace.requests.length > 30) trace.requests = trace.requests.slice(-30);
        } else if (kind === 'submission') {
          trace.lastSubmission = data;
          trace.submissions.push(data);
          if (trace.submissions.length > 30) trace.submissions = trace.submissions.slice(-30);
          if (data && data.runSignature && data.ok) trace.submittedRunSignatures[data.runSignature] = data.completedAt || trace.updatedAt;
        }
        writeTrace(trace);
        return trace;
      }
      ...
      async function submitMappedScores(payload) {
        ...
        const requestRecord = {
          receivedAt,
          pass: PASS,
          reason: payload && payload.reason || 'auto_submit',
        ...
        const result = {
          ok: false,
          pass: PASS,
          status: 'steam_leaderboard_pass_96e_submit_not_started',
          receivedAt,
          completedAt: null,
          liveSteamSubmissionArmed: true,
          autoSubmitEnabled: true,
          ...
          error: null,
          notes: [
            'Automatic wrapper-side final-run submit. No GameScene.js source edits.',
        ...
        try {
          ...
          const trace = readTrace();
          const alreadySubmitted = trace.submittedRunSignatures && trace.submittedRunSignatures[runSignature];
          if (alreadySubmitted && !(payload && payload.force)) {
            result.ok = true;
            result.status = 'steam_leaderboard_pass_96e_duplicate_run_ignored';
          ...
          result.status = result.ok ? 'steam_leaderboard_pass_96e_auto_scores_submitted' : 'steam_leaderboard_pass_96e_auto_submit_incomplete';
          result.completedAt = new Date().toISOString();
        } catch (err) {
          result.ok = false;
          result.status = 'steam_leaderboard_pass_96e_submit_error';
      ...
      try {
        ipcMain.handle(SUBMIT_CHANNEL, async (_event, payload) => submitMappedScores(payload || {}));
      } catch (_) {}
    
      try {
        ipcMain.handle(READ_CHANNEL, async () => {
          const trace = readTrace();
          return Object.assign({ ok: true, status: 'steam_leaderboard_pass_96e_trace_read' }, trace);
        });
      } catch (_) {}
      ...
      try {
        ipcMain.handle(RESET_CHANNEL, async () => {
          ...
          return { ok: true, pass: PASS, status: 'steam_leaderboard_pass_96e_trace_reset', traceFile };
        });
      } catch (_) {}
    })();
    // ---- CHIGGAS_LEADERBOARD_AUTO_SUBMIT_PASS_96E_MAIN END ----
}

module.exports = {
  installLeaderboardAutoSubmitMain
};
